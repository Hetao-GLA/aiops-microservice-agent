"""Run repeatable database-failure trials with recovery checks."""
...
def run_batch(
    *,
    ...
    fault_settings = {
        "database_disconnect": {
            "injector": inject_database_disconnect,
            "health_endpoint": "/health/database",
            "fault_type": "database_connection_failure",
        },
        "service_stop": {
            "injector": inject_service_stop,
            "health_endpoint": "/health/live",
            "fault_type": "service_stopped",
        },
        "http_500": {
            "injector": inject_http_500,
            "health_endpoint": "/health/live",
            "fault_type": "http_500_failure",
        },
    }
    if fault not in fault_settings:
        raise ValueError(f"unsupported fault: {fault}")
    settings = fault_settings[fault]
    selected_injector = injector or settings["injector"]
    ...
    if not wait_until_healthy(
        base_url=base_url,
        endpoint=settings["health_endpoint"],
        timeout_seconds=30,
    ):
        raise RuntimeError("database was not healthy before the batch")
    ...
    for trial_number in range(1, trials + 1):
        ...
        if telemetry_root is not None and run_id is not None:
            telemetry_session = LocalTelemetrySession(
                ...
                expected_fault=str(settings["fault_type"]),
                base_url=base_url,
        ...
        if fault == "http_500":
            injector_arguments["base_url"] = base_url
        try:
            if telemetry_session is not None:
                telemetry_session.start()
            incident_id = selected_injector(**injector_arguments)
            ...
            if not wait_until_healthy(
                ...
                raise RuntimeError(
                    f"system did not recover after trial {trial_number}"
                )
            if telemetry_session is not None:
    ...
    result: dict[str, object] = {
        ...
        "trials_completed": len(incidents),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run repeatable fault trials.")
    parser.add_argument(
        "--fault",
        choices=["database_disconnect", "service_stop", "http_500"],
        default="database_disconnect",
    )
    ...
    parser.add_argument(
        "--output",
        default="data/results/latest-batch.json",
    )
    parser.add_argument(
        "--telemetry-root",
